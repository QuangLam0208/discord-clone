...
public class FileValidationService {
    ...
    public void validateImageDimensions(byte[] fileBytes, String contentType)
        ...
        try (ByteArrayInputStream bais = new ByteArrayInputStream(fileBytes);
            ...
            if (!readers.hasNext()) {
                // Với WEBP nếu thiếu plugin sẽ không có reader -> báo lỗi rõ ràng
                throw new IOException("Không tìm thấy ImageReader cho content type: " + contentType +
                        ". Nếu là image/webp, hãy thêm TwelveMonkeys ImageIO plugin.");
            }
            ...
            try {
                ...
                if (width > MAX_IMAGE_WIDTH) {
                    throw new IllegalArgumentException(String.format("Image width %d vượt quá %d pixels", width, MAX_IMAGE_WIDTH));
                }
                if (height > MAX_IMAGE_HEIGHT) {
                    throw new IllegalArgumentException(String.format("Image height %d vượt quá %d pixels", height, MAX_IMAGE_HEIGHT));
                }
                if (totalPixels > MAX_IMAGE_PIXELS) {
                    throw new IllegalArgumentException(String.format("Image size %d pixels vượt quá %d pixels", totalPixels, MAX_IMAGE_PIXELS));
                }
