const DMUI = (() => {
  ...
  function renderMessages(list, currentUserId, displayedMsgIdsSet, friendInfo, relationshipStatus, avatarMap, friendName) {
    ...
    if (relationshipStatus === 'BLOCKED') {
        messagesToRender = [];
        blockNoticeHtml = `<div style="display: flex; justify-content: center; margin: 20px 0;"><span style="background-color: #36393f; padding: 8px 12px; border-radius: 4px; color: #dcddde; font-size: 14px; border: 1px solid #202225;">Tin nhắn đã bị ẩn vì bạn đang chặn người dùng này.</span></div>`;
    }
