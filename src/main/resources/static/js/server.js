...
async function loadServers() {
    // Note: elements.sidebar is defined in home.js or init loop. 
    // We assume 'elements' is global or we re-query via helper if needed. 
    // BUT since we are splitting, better re-query or rely on global `elements`.
    // For now, let's re-query to be safe in module, or assume 'elements' is available globally.
    // Based on plan, we can use global `elements` defined in home.js (it runs first).
